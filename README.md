...
楽天市場商品検索APIから育児商品を取得し、品質合格した5商品を楽天ROOM向け投稿候補としてGoogleスプレッドシートへ追記します。候補が品質条件を満たさない場合は、次の商品へ補充選定を続けます。
...
2. URLと近似商品名で既存商品を除外
3. スコアと直近30日の商品タイプを使って候補の優先順を決定
4. 商品名をクリーニング
5. 商品タイプを細分類
...
10. 朝・昼・晩の3枠すべてに`ready`が1件ずつ揃うまで、次候補の生成・品質チェックを続ける
11. 3枠が揃った実行だけを投稿候補シートへ追記し、各行へ`morning`・`noon`・`evening`を割り当てる
11. `ready`または`needs_review`としてSheetsへ追記
...
生成レポートの`post_slot`と現在の時間帯が一致する商品だけを投稿します。朝・昼・晩の3枠がすべて
品質合格しない実行は失敗扱いになり、部分的な`ready`は投稿候補シートへ追記されません。
時刻を過ぎた当日枠の回復時だけ`ROOM_FORCE_POST_SLOT=morning`のように枠を明示できます。
通常タスクでは設定せず、台帳の枠予約により回復実行でも二重投稿を防ぎます。
...
完了・未完了にかかわらず、日付・件数・失敗数だけを公開用`routine-state`ブランチへ更新します。
候補ID、ROOM URL、認証情報は公開しません。GitHub Pagesの上段「自動処理実績」はこの集計を表示し、
下段「手動チェック」は従来どおりブラウザ別のlocalStorage表示です。05:10処理後に加えて06:45にも
同じタスクを実行し、処理済みなら実操作せず公開集計だけを再送します。
...
公開ユーザー検索は画面表示ありのChromeで検索欄を送信して候補を補充するため、Windowsタスクは
`--headful`付きで登録します。

タスク登録後の既定時刻は、フォロー・いいねが5:10、投稿が8:00・12:00・19:00です。
さらに生成監視を7:30、投稿確認・当日補填を8:30・12:30・19:30に実行します。
