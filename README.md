...
- 200〜350文字程度
- `【商品名＋ベネフィット】` と `【投稿文】` の構成
- 冒頭は育児の悩みや共感から入る
- なぜその年齢に合うのかを入れる
- 数ある商品の中でこの商品を選ぶ理由を入れる
- 商品名が長い場合は自然に短縮する
- レビュー件数、評価、総合スコアを本文の中心にしない
- 子どもにとってのベネフィット、親にとってのベネフィットを優先する
- パパ目線の自然な口語体にする
- 購入前確認点は毎回本文に入れず、サイズ、対象年齢、誤飲リスク、設置場所、配送条件が重要な商品だけ自然に入れる
- 最後はどんな家庭におすすめかで締める
- ハッシュタグは楽天ROOM向けに5個
...
1. 購入意欲タグ: `#買ってよかった`, `#おすすめ品`, `#プレゼントにおすすめ` など
2. 商品カテゴリタグ: `#知育玩具`, `#育児便利グッズ`, `#ベビーグッズ` など
3. 悩み解決タグ: `#おうち遊び`, `#イヤイヤ期対策`, `#育児時短` など
4. ターゲットタグ: `#2歳育児`, `#3歳育児`, `#共働き育児` など
5. ブランドタグ: `#とらパパ厳選`

プレゼント訴求が強い商品では、ターゲットタグより `#出産祝い` や `#1歳誕生日プレゼント` などのプレゼントタグを優先します。
...
あわせて、`遊びながら指先を使える`、`お風呂時間を楽しい時間に変えやすい`、`親の準備や片づけの負担を減らしやすい` のような1文ベネフィットを出力します。
