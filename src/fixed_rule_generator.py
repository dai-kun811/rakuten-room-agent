...
def clean_product_name(product: Product) -> tuple[str, str, list[str]]:
    ...
    if product_type == "unknown" or not short_label:
        errors.append("商品タイプまたは短縮名を確定できない")
...
def build_candidate(
    ...
    scene = pattern.scene.format(feature=feature, checks=checks)
    closing = pattern.closing.format(feature=feature, checks=checks)
    ...
    post = GeneratedPost(
        title=pattern.title,
        body=body,
        hashtags=hashtags_for(attributes, body=body, title=pattern.title),
        analysis=analysis,
        quality=QualityScore(),
...
def validate_post(
    ...
    if unconfirmed:
        errors.append(f"未確認属性を使用: {','.join(sorted(unconfirmed))}")
